...
class QtImageViewer(QGraphicsView):
    ...
    def updateViewer(self):
        ...
        if len(self.zoomStack) and self.sceneRect().contains(self.zoomStack[-1]):
            ...
            print('fitting in view')
            self.fitInView(self.sceneRect(), self.aspectRatioMode)  # Show entire image (use current aspect ratio mode).
            print('fitting in view done')

    def resizeEvent(self, event):
        """ Maintain current zoom on resize.
        """
        self.resizes += 1
        print(f'resize event: {self.resizes}')
        self.updateViewer()
    ...
    def ext_resetzoom(self, event):
        """ Show entire image. This method is called from a user of this object,
            for example, from a pushbutton.
        """
        if self.canZoom:
            self.zoomStack = []  # Clear zoom stack.
            self.updateViewer()
